        ...
        {receipt && (
          <section className="max-w-6xl mx-auto px-6 pb-24 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-6">
               <div className="bg-white p-6 rounded-2xl shadow-sm border border-outline-variant/30 flex flex-col md:flex-row justify-between gap-4 md:items-center bg-gradient-to-br from-white to-surface-container-low">
                 <div>
                    <p className="text-sm text-on-surface-variant font-bold mb-1">Status Pengiriman</p>
                    <h2 className="text-3xl font-black font-headline text-primary uppercase">{getStatusLabel(receipt.currentStatus)}</h2>
                    <p className="text-sm mt-1">{formatDate(receipt.createdAt)}</p>
                 </div>
                 <div className="flex flex-col gap-2">
                   <button 
                      onClick={() => handlePrint()}
                      className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md font-bold text-sm shadow-md transition-colors"
                   >
               ...
               <div className="relative pl-6 border-l-2 border-outline-variant mt-2 space-y-10 py-2">
                 {receipt.statusHistory.map((history, idx) => (
                   <div key={idx} className="relative group">
                     {/* Circular Bullet */}
                     <div className={`absolute -left-8.25 w-4 h-4 rounded-full ring-4 ring-white ${idx === 0 ? 'bg-green-500' : 'bg-outline-variant'}`}>
                     ...
                     <div className={`transition-all ${idx !== 0 && 'opacity-60 group-hover:opacity-100'}`}>
                       ...
                       {history.description && (
                         <p className="text-on-surface-variant text-sm mt-1 uppercase">
                           {history.description}
                         </p>
                       )}
                       {history.location && (
...
export default function CekResiClientPage() {
  return (
    <PublicLayout>
      <Suspense fallback={
        <div className="min-h-screen flex items-center justify-center">
